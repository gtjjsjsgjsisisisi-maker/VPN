-- ==================== EXC HUB (RAYFIELD LOCKED CENTER - DELTA MOBILE) ====================
...
-- ==================== OFFICIAL MOBILE RAYFIELD LIBRARY ====================
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()

local Window = Rayfield:CreateWindow({
    Name = "EXC HUB | Mobile Delta",
...
-- ==================== LOCK UI POSITION IN CENTER ====================
task.spawn(function()
    task.wait(0.5)
    local safeParent = GetSafeParent()
    local function enforceCenterLock(container)
    ...
    enforceCenterLock(safeParent)
    enforceCenterLock(game:GetService("CoreGui"))
end)

-- ปุ่มลอยสลับเปิด/ปิด UI หน้าจอมือถือ
...
ToggleBtn.MouseButton1Click:Connect(function()
    pcall(function()
        ...
        for _, v in pairs(parentContainer:GetChildren()) do
            if v:IsA("ScreenGui") and (v.Name:lower():find("rayfield") or v:FindFirstChild("Main")) then
...
Rayfield:Notify({
    Title = "EXC HUB",
    Content = "ล็อค UI อยู่ตรงกลางหน้าจอเรียบร้อย!",
    Duration = 5
})